...
def pairing_margin_ranking_loss(
    ...
    if len(losses) == 0:
        # Return a zero that still requires grad so backward() works
...
def train_one_epoch_tokenmix_align_full(
    ...
    for batch in tqdm(dataloader, desc=f"Epoch {epoch} [train TokenMix+Align full COCO]"):
...
def main():
    ...
    train_loader = DataLoader(
        train_dataset,
        batch_size=128,       # adjust if you get OOM
        shuffle=True,
        num_workers=4,
    ...
    num_epochs = 10  # start with 5; can go to 10 later

    for epoch in range(1, num_epochs + 1):
        avg_loss = train_one_epoch_tokenmix_align_full(
            model=model,
            augmentor=augmentor,
        ...
        print(f"Epoch {epoch}: avg train loss (TokenMix+Align full COCO) = {avg_loss:.4f}")

    os.makedirs("checkpoints", exist_ok=True)
    torch.save(model.state_dict(), "checkpoints/retrieval_tokenmix_align_full_coco_model.pt")
    torch.save(augmentor.state_dict(), "checkpoints/retrieval_tokenmix_align_full_coco_augmentor.pt")
    torch.save(pairing_net.state_dict(), "checkpoints/retrieval_tokenmix_align_full_coco_pairing.pt")
    print("Saved TokenMix+Align FULL COCO model, augmentor, and pairing network.")
