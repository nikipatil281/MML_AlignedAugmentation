...
def choose_mixing_partners(pairing_emb: torch.Tensor, top_k: int = 5) -> torch.Tensor:
    """
    pairing_emb: [B, P] normalized
    Returns:
        partner_indices: [B] long tensor, partner index j for each i
    """
    device = pairing_emb.device
    B = pairing_emb.size(0)

    sim = pairing_emb @ pairing_emb.t()  # [B, B]
    # Avoid self-selection by strongly penalizing diagonal
...
def compute_modality_mix_weights(pairing_emb: torch.Tensor, partner_indices: torch.Tensor):
    """
    Compute λ_img, λ_txt based on similarity between anchor and partner.
    ...
    sim_vals = torch.sum(pairing_emb * partner_emb, dim=-1)  # cosine sim ∈ [-1, 1]

    # Map sim from [-1,1] -> [0.25, 0.75]
    lambda_img = 0.5 + 0.25 * sim_vals
    lambda_img = torch.clamp(lambda_img, 0.25, 0.75)
    lambda_txt = 1.0 - lambda_img

    return lambda_img.view(B, 1), lambda_txt.view(B, 1)
...
def train_one_epoch_hybrid(
    ...
    for batch in tqdm(dataloader, desc=f"Epoch {epoch} [train Hybrid Premix]"):
        ...
        # ======================================================
        # 1) Update pairing_net ψ with label-based ranking loss
        # ======================================================
        L_pair = pairing_margin_ranking_loss(pairing_emb, category_multi_hot, margin=0.2)

        optim_pairing.zero_grad()
        L_pair.backward(retain_graph=True)  # keep graph for pairing_emb reuse
        optim_pairing.step()

        # ======================================================
        # 2) Semantic pre-mixing using pairing_emb
        # ======================================================
        partner_indices = choose_mixing_partners(pairing_emb.detach(), top_k=5)  # [B]

        lambda_img, lambda_txt = compute_modality_mix_weights(pairing_emb.detach(), partner_indices)

        img_partner = img_emb[partner_indices]  # [B, D]
        txt_partner = txt_emb[partner_indices]  # [B, D]

        img_mix = lambda_img * img_emb + (1.0 - lambda_img) * img_partner
        txt_mix = lambda_txt * txt_emb + (1.0 - lambda_txt) * txt_partner

        # normalize mixed embeddings
        img_mix = nn.functional.normalize(img_mix, p=2, dim=-1)
        txt_mix = nn.functional.normalize(txt_mix, p=2, dim=-1)

        r_mix = torch.cat([img_mix, txt_mix], dim=-1)  # [B, 2D]

        # ======================================================
        # 3) Update augmentor φ on mixed representations
        # ======================================================
        r_mix_det = r_mix.detach()
        r_aug_for_aug = augmentor(r_mix_det)  # [B, 2D]

        img_aug_for_aug = r_aug_for_aug[:, :D]
        txt_aug_for_aug = r_aug_for_aug[:, D:]
        img_aug_for_aug = nn.functional.normalize(img_aug_for_aug, p=2, dim=-1)
        txt_aug_for_aug = nn.functional.normalize(txt_aug_for_aug, p=2, dim=-1)
        ...
        # ======================================================
        # 4) Update retrieval model θ with original + mixed+aug
        # ======================================================
        with torch.no_grad():
            r_aug_for_model = augmentor(r_mix.detach())
        img_aug = r_aug_for_model[:, :D]
        txt_aug = r_aug_for_model[:, D:]
        img_aug = nn.functional.normalize(img_aug, p=2, dim=-1)
        txt_aug = nn.functional.normalize(txt_aug, p=2, dim=-1)
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        avg_loss = train_one_epoch_hybrid(
            model=model,
            augmentor=augmentor,
        ...
        print(f"Epoch {epoch}: avg train loss (Hybrid Premix) = {avg_loss:.4f}")

    os.makedirs("checkpoints", exist_ok=True)
    torch.save(model.state_dict(), "checkpoints/retrieval_hybrid_premix_model.pt")
    torch.save(augmentor.state_dict(), "checkpoints/retrieval_hybrid_premix_augmentor.pt")
    torch.save(pairing_net.state_dict(), "checkpoints/retrieval_hybrid_premix_pairing.pt")
    print("Saved Hybrid Premix model, augmentor, and pairing network.")
