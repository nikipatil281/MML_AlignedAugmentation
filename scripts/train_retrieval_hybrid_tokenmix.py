...
def compute_modality_mix_weights(pairing_emb: torch.Tensor, partner_indices: torch.Tensor):
    ...
    sim_vals = torch.sum(pairing_emb * partner_emb, dim=-1)  # cosine in [-1,1]

    lambda_img = 0.5 + 0.25 * sim_vals
    lambda_img = torch.clamp(lambda_img, 0.25, 0.75)
    lambda_txt = 1.0 - lambda_img

    return lambda_img.view(B, 1, 1), lambda_txt.view(B, 1, 1)  # broadcast over tokens/patches


def train_one_epoch_tokenmix(
    ...
    for batch in tqdm(dataloader, desc=f"Epoch {epoch} [train TokenMix]"):
        ...
        # ---- Pairing embedding from globals ----
        pairing_emb = pairing_net(img_global.detach(), txt_global.detach())  # [B, P]

        # 1) Update pairing_net with label-aware loss
        ...
        # 2) Choose semantic partners & compute λs
        ...
        # lambda_*: [B,1,1], broadcasts over tokens/patches
        ...
        # 5) Update retrieval model with original + mixed+augmented
        with torch.no_grad():
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        avg_loss = train_one_epoch_tokenmix(
            model=model,
            augmentor=augmentor,
        ...
        print(f"Epoch {epoch}: avg train loss (TokenMix) = {avg_loss:.4f}")

    os.makedirs("checkpoints", exist_ok=True)
    torch.save(model.state_dict(), "checkpoints/retrieval_tokenmix_model.pt")
    torch.save(augmentor.state_dict(), "checkpoints/retrieval_tokenmix_augmentor.pt")
    torch.save(pairing_net.state_dict(), "checkpoints/retrieval_tokenmix_pairing.pt")
    print("Saved TokenMix model, augmentor, and pairing network.")
